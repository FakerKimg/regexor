...
class disj(connective):
    ...
    def support(self):
        p = self.arg1.support()
        p.update(self.arg2.support())
        return p

    def supportlast(self):
        p = self.arg1.supportlast()
        p.update(self.arg2.supportlast())
        return p
...
class option(regexp):
    ...
    def snf(self, _hollowdot=False):
        ...
        return option(self.arg.snf(True),self.Sigma)
...
class star(regexp):
    ...
    def support(self):
        return self._odot(self.arg.support())

    def supportlast(self):
        return self._odot(self.arg.supportlast())
...
class concat(connective):
    ...
    def support(self):
        p = self.arg2._odot(self.arg1.support())
        p.update(self.arg2.support())
        return p

    def supportlast(self):
        p = self.arg2.supportlast()
        if self.arg2.ewp():
            p.update(self.arg2._odot(self.arg1.supportlast()))
        return p
...
class sigmaS(specialConstant):
    """Class that represents the complement of the emptyset set"""
...
class sigmaP(specialConstant):
    """Class that represents the complement of the emptyset word"""
    def __hash__(self):
        return hash(type(self))
...
class sconcat(sconnective):
    ...
    def support(self):
        ...
        head = self.head()
        tail = self.tail()
        pi = concat_set(head.support(), tail)
        return pi | tail.support()
...
class snot(regexp):
    ...
    def support1(self):
        ...
        pi = set()
        while next:
            next_neg = negation_set(next, self.Sigma)
            re = next_neg.pop()
            pi.add(re)
            next = power.next()
        next_neg = negation_set(next, self.Sigma)
...
class Parse_x(Yappy):
    ...
    def __init__(self, no_table=0, table='.tablexreg'):
        ...
        grammar = [("r", ["r", "|", "c1"], self.DisjRule),
                   ...
                   ("b", [SigmaS], self.BaseRule),
                   ("b", [SigmaP], self.BaseRule)]
        tokenize = [("\s+", ""),
    ...
    def DisjRule(self, lst, _):
        """
        :param lst:
        :return: """
        return lst[0]._plus(lst[2])

    def ConcatRule(self, lst, _):
        """
        :param lst:
        :return:"""
...
def str2sre(s, sigma = None, no_table=1):
    """ Reads a regexp from string.

    :arg str s:  the string representation of the regular expression
    :arg int no_table:
    :arg sigma: alphabet of the regular expression
    :type sigma: iterable of symbols
    :rtype: regexp"""
    s = re.sub("\s+", "", s)
    d = Parse_x(no_table)
    try:
...
def rpn2xre(s):
    ...
    :arg s: the RPN representation of the regular expression
    :type s: string
    :rtype: xre"""
...
def _rpn2xre(s, i):
    ...
    if s[i] == "*":
        (i1, arg1) = _rpn2xre(s, i + 1)
        if type(arg1) is sstar:
            return i1, arg1
        else:
            return i1, sstar(arg1)
    if s[i] == "@":
        return i + 1, epsilon()
    else:
        return i + 1, atom(s[i])
...
def to_xdisj(r):
    ...
    :param r:
    :return:"""
    return sdisj(frozenset(_to_xdisj(r)))
...
def _to_xdisj(r):
    ...
    if type(r.arg1) is disj:
        if type(r.arg2) is disj:
            return _to_xdisj(r.arg1) | _to_xdisj(r.arg2)
        else:
            return _to_xdisj(r.arg1) | {to_x(r.arg2)}
    else:
        if type(r.arg2) is disj:
            return {to_x(r.arg1)} | _to_xdisj(r.arg2)
        else:
            return {to_x(r.arg1)} | {to_x(r.arg2)}
...
def _to_xconcat(r):
    ...
    if type(r.arg1) is concat:
        if type(r.arg2) is concat:
            return _to_xconcat(r.arg1) + _to_xconcat(r.arg2)
        elif type(r.arg2) is epsilon:
            return _to_xconcat(r.arg1)
        else:
            return _to_xconcat(r.arg1) + (to_x(r.arg2),)
    else:
        if type(r.arg2) is concat:
            return (to_x(r.arg1),) + _to_xconcat(r.arg2)
        elif type(r.arg2) is epsilon:
            return to_x(r.arg1),
        else:
            return (to_x(r.arg1),) + (to_x(r.arg2),)
...
class ParseReg1(Yappy):
    ...
    def ShuffleSemRule(self,lst, context=None):
        """
        :param lst:
        :param context:
        :return:"""
    ...
    def ConjSemRule(self, lst, context=None):
        """
        :param lst: argument list
        :param context:
        :return: regular expression
    ...
    def OptionSemRule(self, lst, context=None):
        if "sigma" in context:
            sigma = context["sigma"]
        else:
            sigma = None
...
class ParserRPN(Yappy):
    def __init__(self, no_table=1, table='.tableshurpn'):
        ...
        # """reg -> + reg reg {{ self.OrSemRule}} |
        #             {0:s} reg reg {{ self.ShuffleSemRule}} |
        #           . reg reg {{ self.ConcatSemRule }} | * reg  {{ self.StarSemRule }} |
        #         {1:s} {{ self.BaseSemRule}} | {2:s} {{ self.BaseSemRule}} |
        #        id {{ self.BaseSemRule }} ;""".format(Shuffle,Epsilon,EmptySet)
        tokenize = [("\s+", ""),
                    (Epsilon, lambda x: (x, x)),
                    (EmptySet, lambda x: (x, x)),
                    (Shuffle, lambda x: (x, x)),
