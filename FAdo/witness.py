...
def shuffle_ReBC(n=3):
    """ Withess for Shuffle toDFA
    """

    if n < 2:
        raise TestsError("at least n=2")
    rs = star(concat(atom('a0'), atom('b0')))
    for i in range(1, n):
        rs = shuffle(rs,star(concat(atom('a'+str(i)),atom('b'+str(i)))))
    return rs
...
def shuffle_ReBCF(n=3):
    ...
    if n<2:
        raise TestsError("at least n=2")
    rs = concat(atom('a0'),atom('b0'))
    for i in range(1,n):
        rs = shuffle(rs,concat(atom('a'+str(i)),atom('b'+str(i))))
    return rs
...
def shuffle_NFABC(n=3):
    ...
    if n<2:
        raise TestsError("at least n=2")
    ns = star(concat(atom('a0'), atom('b0'))).toNFA()
    for i in range(1, n):
        ni = star(concat(atom('a'+str(i)), atom('b'+str(i)))).toNFA()
        ns = ns.shuffle(ni)
    return ns
