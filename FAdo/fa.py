...
class SemiDFA(Drawable):
    ...
    def __init__(self):
        """
        :var list States: list of states
        :var dict delta: transition function
...
class FA(Drawable):
    ...
    def __init__(self):
        """
        :var list States: set of states
        :var set Sigma: alphabet set
        :var int Initial: the initial state index
        :var set Final: set of final states indexes
        :var dict delta: the transition function"""
        self.States = []
        self.Sigma = set()
        self.Initial = None
        self.Final = set()
    ...
    def dotDrawState(self, sti, sep="\n", strict=False, maxLblSz=6):
        ...
        if sti in self.Final:
            return "node [shape = doublecircle]; \"{0:s}\";".format(graphvizTranslate(str(self.States[sti])), sep)
        else:
            return "node [shape = circle]; \"{0:s}\";{1:s}".format(graphvizTranslate(str(self.States[sti])), sep)
    ...
    @abstractmethod
    def _deleteRefInDelta(self):
        pass

    @abstractmethod
    def _deleteRefInitial(self):
        pass
    ...
    def addFinal(self, stateindex):
        ...
        :param int stateindex: index of the new final state"""
    ...
    def dotFormat(self, size="20,20", direction="LR", sep="\n", strict=False, maxLblSz=6):
        ...
        niStates = [i for i in range(len(o.States)) if i != o.Initial]
        s += o.dotDrawState(o.Initial)
        s += "dummy -> \"{0:s}\"{1:s}".format(str(o.States[o.Initial]), sep)
        for sti in niStates:
...
class OFA(FA):
    ...
    def dotDrawTransition(st1, label, st2, sep="\n"):
        ...
        return "\"{0:s}\" -> \"{1:s}\" [label = \"{2:s}\"];{3:s} ".format(graphvizTranslate(st1),
                                                                          graphvizTranslate(st2), label, sep)
    ...
    def deleteStates(self, del_states):
        ...
        :param list del_states: states to be deleted
        """
        pass
...
class NFA(OFA):
    ...
    def succintTransitions(self):
        ...
        :rtype: list"""
        #TODO: Check coherence between this and DFA version
        foo = dict()
        for s in self.delta:
    ...
    def reorder(self, dicti):
        ...
        :param dict dicti: state name reorder
    ...
    def reverseTransitions(self, rev):
        ...
        :param NFA rev: NFA in which the reverse function will be stored"""
        for s in self.delta:
    ...
    def dotFormat(self, size="20,20", direction="LR", sep="\n", strict=False, maxLblSz=6):
        ...
        for si in o.Initial:
            ...
            s += "node [shape = point]; \"dummy{0:s}\"{1:s}".format(sn, sep)
            s += o.dotDrawState(si)
            s += "\"dummy{0:s}\" -> \"{1:s}\";{2:s}".format(sn, str(o.States[si]), sep)
        niStates = [i for i in range(len(o.States)) if i not in o.Initial]
        for sti in niStates:
            s += o.dotDrawState(sti)
        for si in o.succintTransitions():
            s += o.dotDrawTransition(si[0], si[2], si[1])
        s += "}}{0:s}".format(sep)
        return s
    ...
    def wordImage(self, word, ist=None):
        ...
        :param int ist: starting state index (or set of)
    ...
    def product(self, other):
        ...
        :param NFA other: the other automata
        :rtype: NFA
    ...
    def hasTransitionP(self, state, symbol=None, target=None):
        ...
        :param int state: source state
        :param str symbol: optional transition symbol
        :param int target: optional target state
    ...
    def equivReduced(self, equiv_classes):
        ...
        :param UnionFind equiv_classes: Equivalence classes
    ...
    def epsilonPaths(self, start, end):
        ...
        :param int start: start state
        :param int end: end state
    ...
    def enumNFA(self, n=None):
        ...
        :param int n: highest lenght or all words if finite
...
class NFAr(NFA):
    ...
    def homogenousP(self, inplace=False):
        ...
        :arg bool inplace: if True performs epsilon transitions elimination
...
class DFA(OFA):
    ...
    def delTransition(self, sti1, sym, sti2, _no_check=False):
        ...
        :param bool _no_check: use unsecure code?
        :param int sti1: state index of departure
        :param int sti2: state index of arrival
        :param str sym: symbol consumed
    ...
    def distMin(self):
        ...
        if sz == 1:
            return fl.FL()
        distList = fl.FL(Sigma=self.Sigma)
                ...
                try:
                    for c in self.delta[initial]:
                        if self.delta[initial][c] == final:
                            if not r.emptyP():
                                r = reex.disj(r, reex.atom(c, copy(self.Sigma)))
                            else:
                                r = reex.atom(c, copy(self.Sigma))
                except KeyError:
                    pass
                return r.reduced()
        else:
...
class GFA(OFA):
    ...
    def addTransition(self, sti1, sym, sti2):
        ...
        try:
            self.addSigma(sym)
            sym = reex.atom(sym, copy(self.Sigma))
...
class EnumL(object):
    """Class for enumerate FA languages
        ...
        :var FA aut: Automaton of the language
        :var dict tmin: table for minimal words for each s in aut.States
        :var list Words: list of words (if stored)
        :var list Sigma: alphabet
        :type stack: deque
